...
class NodeList:
    ...
    def merge_same_line(self) -> None:
        ...
        for line in duplicate_lines:
            duplicates = sorted(
                [n for n in self.nodes if n.line == line], key=lambda x: x.subroutine
            )  # assumption is that one must be a subroutine
                ...
                for node in ms_nodes:
                    for lj in node.line_jumps:
                        if not (lj in cond_jump_node.line_jumps or lj == cond_jump_node.line):
                            # a valid line jump that needs to be added to the correct node
                            cond_jump_node.line_jumps.insert(
                                0, lj
                            )  # assumption is that a GOSUB statement is before a conditional jump, hence add it before the conditional jumps
