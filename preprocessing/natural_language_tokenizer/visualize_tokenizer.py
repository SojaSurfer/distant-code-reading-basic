...
def save_tokens_html(
    sentences: list[str],
    tokenizerNames: list[str],
    out_path: Path,
    additionalTokenizer: tuple[AutoTokenizer, str] = None,
) -> None:
    body = ""
    ...
    if isinstance(tokenizerNames[0], str):
        tokenizerList = [
            AutoTokenizer.from_pretrained(name, cache_dir=MODEL_PATH, batched=True) for name in tokenizerNames
        ]
...
if __name__ == "__main__":
    ...
    save_tokens_html(
        randomCodeExamples,
        modelNames,
        ROOT / "tokenizer_visualization_.html",
        additionalTokenizer=[trainedTokenizer, trainedTokenizerPath.name],
    )
