"""
The script extracts & converts d64 files into readable txt files.
"""
...
def extractD64Files(vice_bin_path:Path, d64_path:Path, extraction_dir:Path) -> None:
    """The function extracts all d64 files with c1541 from a source dir to a dest dir.
    ...
    for file in d64_path.iterdir():
        ...
        if file.suffix == '.d64':
            ...
            for extractedFile in Path('.').iterdir():
                if extractedFile.is_file() and not extractedFile in cwd_files:
                    shutil.move(extractedFile, extraction_path / extractedFile.name)
...
def convertD64Files(vice_bin_path:Path, extraction_dir:Path) -> None:
    """The function converts all files without a suffix (c64 source code) to plain txt files using petcat.
...
if __name__ == '__main__':
    ...
    convertD64Files(vice_bin_path, extraction_dir)
